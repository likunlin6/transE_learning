...
#数据加载
def data_loader(file1,file2,file3,file4):
    print("load file...")
    ...
    with open(file2,'r') as f1 ,open(file3,'r') as f2:
        lines1 = f1.readlines() #entity2id
        lines2 = f2.readlines() #relation2id

        #把实体id和序号存入字典relations2id
        for line in lines1:
            line = line.strip().split('\t')#制表位
            if len(line) != 2:
                continue
            entities2id[line[0]] = line[1]
            entity.append(int(line[1]))

        # 把关系id和序号存入字典entities2id
        for line in lines2:
            line = line.strip().split('\t')#制表位
            if len(line) != 2:
                continue
    ...
    #open,io.open,codecs.open都可以用，但是如果用codecs.open呢， 那么不管是python2.X，还是Python3，哈哈，那都是兼容的了，包括可以兼容中文的情况。
    #读入训练集
    with codecs.open(file1,'r') as f :
        ...
        for line in content:
            ...
            h_ = int(entities2id[triple[0]])#训练集中的头实体
            r_ = int(relations2id[triple[1]])#关系
            t_ = int(entities2id[triple[2]])#尾实体

            triple_list.append([h_,r_,t_])#加入三元组
    ...
    #创造负样本（错误三元组）的时候，这里使计算了平均尾节点数 hpt 和平均头结点数tph,
    # tph 表示每一个头结对应的平均尾节点数 ,hpt 表示每一个尾节点对应的平均头结点数
...
class E(nn.Module):
    def __init__(self,entity_num,relation_num,dim,margin,norm,C):
        ...
        self.C = C
        #实体嵌入
        self.ent_embedding = nn.Embedding(num_embeddings=self.entity_num,embedding_dim=dim).cuda()
        #关系嵌入
        self.rel_embedding = nn.Embedding(num_embeddings=self.relation_num,embedding_dim=dim).cuda()
        #损失函数
    ...
    def distance(self,h,r,t):
        ...
        # 就需要sum的结果也是2维张量 因此需要使用keepdim= True保持张量的维度不变
...
if __name__ == '__main__':
    ...
    file_1 = "WN18\\wordnet-mlj12-train.txt"
    file_2 = "WN18\\entity2id.txt"
    file_3 = "WN18\\relation2id.txt"
    file_4 = "WN18\\wordnet-mlj12-valid.txt"

    entity_set , relation_set , triple_list ,valid_triple_list = data_loader(file_1,file_2,file_3,file_4)

    #这两个文件的含义是？
    file_5 = "WN18_TransE_entity_50dim_batch1414"
    file_6 = "WN18_TransE_relation_50dim_batch1414"

    #测试集
    file_7 = "WN18\\wordnet-mlj12-test.txt"
    ...
    transE.insert_test_data(file_5, file_6, file_7)
